...
public class TestSingleRedis {
    ...
    public static void setUpBeforeClass() {
        //单个节点
        jedis = new Jedis("192.168.80.30",6379);

        //分片
        List<JedisShardInfo> shardInfos = Arrays.asList(new JedisShardInfo("192.168.80.30",6379));
        ...
        jedisPool = new ShardedJedisPool(cfg,shardInfos);
    }
    ...
    public void testString() {
        //-----添加数据----------
        jedis.set("name","lsieun");
        System.out.println(jedis.get("name"));

        //拼接
        jedis.append("name", " is learning!");
        System.out.println(jedis.get("name"));
        ...
        //设置多个键值对
        jedis.mset("name","lucy","age","12","sex","female");
    ...
    public void testMap() {
        ...
        System.out.println(jedis.hkeys("user"));//返回map对象中的所有fields
    ...
    public void testSet(){
        //添加
        jedis.sadd("user","xiao_hong");
        jedis.sadd("user","xing_ming");
        jedis.sadd("user","zhang_san");
        jedis.sadd("user","li_si");
        jedis.sadd("user","qin_fei_fei");
        //移除noname
        jedis.srem("user","qin_fei_fei");
        System.out.println(jedis.smembers("user"));//获取所有加入的value
        System.out.println(jedis.sismember("user", "qin_fei_fei"));//判断 who 是否是user集合的元素
